setwd(dir = "/media/zx/HDD1/cooperation/liyr/result/dyngen/result_0921/eval/cell_level/")
...
file_all <- c("eval_global.csv","eval_incluster.csv","eval_direction.csv","cosine_sim_velocity.csv","cosine_sim_velocity_pca2.csv","cosine_sim_trans_matrix.csv")
...
for (i in simuID_list){
    ...
    bb <- read.xlsx(xlsxFile = "/media/zx/HDD1/cooperation/liyr/result/dyngen/result_0921/eval/data/cosine_sim_velocity_pca2-30.xlsx",sheet = i,rowNames = T)
...
data_velocity <- read.csv(file = "data/cosine_sim_velocity.csv",header = T,row.names = 1)
...
#     tt <- c()    
#     for (i in 1:29){
#         tt <- c(tt,cor(x = unlist(list_res2[[i]][j,]),y = unlist(data_velocity[j,]),use = "na.or.complete"))
#     }
#     xx[[j]] <- tt
# }
...
pdf(file = "velocity_pca_2-30_split_datasets_ALL_cosine_similarity2.pdf",width = 10,height = 6)
