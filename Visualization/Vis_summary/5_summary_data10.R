...
####------read data-----------------------
metrics <- fread('/media/liyaru/LYR/Benchmark/datasets10/global_1205.csv') %>% as.data.frame()

colnames(metrics) <- c('Method','Pancreatic Endocrinogenesis',
...
metrics <- metrics[,c('Method',datasets)]

metrics[] <- lapply(metrics, function(x) gsub('"', '', x))
metrics[,2:11] <- lapply(metrics[,2:11], as.numeric)
...
# add median
metrics$Median <- apply(metrics[,2:11], 1, median, na.rm = TRUE)
metrics$Mean <- apply(metrics[,2:11], 1, mean, na.rm = TRUE)

fwrite(metrics,'/media/liyaru/LYR/Benchmark/datasets10/global_1205_reorder.csv')

metrics <- as.data.frame(metrics)

# normalize by group
x_min <- min(metrics[,2:11], na.rm = TRUE)
x_max <- max(metrics[,2:11], na.rm = TRUE)
metrics[,2:11] <- (metrics[,2:11] - x_min) / (x_max - x_min)


# normalize mean and median
# metrics[,12:13] <- apply(metrics[,12:13],2,normalize_by_col)
x_min <- min(metrics[,12:13], na.rm = TRUE)
x_max <- max(metrics[,12:13], na.rm = TRUE)
metrics[,12:13] <- (metrics[,12:13] - x_min) / (x_max - x_min)
...
ggsave("/media/liyaru/LYR/Benchmark/Fig/5.data10_global_summary.pdf",
       g, 
...
metrics <- t(metrics[,2:11]) %>% as.data.frame()

metrics$`Best performance` <- apply(metrics, 1, max,na.rm = TRUE)
...
p <- pheatmap(metrics,cluster_cols = F,
...
pdf("/media/liyaru/LYR/Benchmark/Fig/5.data10_heatmap_new.pdf",width = 10,height = 4)
p
dev.off()
