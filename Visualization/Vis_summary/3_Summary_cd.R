...
####------read data-----------------------
metrics <- fread('/media/liyaru/LYR/Benchmark/simu_celldancer/celldancer_cosine_sim_all.csv') %>%
  as.data.frame()
...
colnames(metrics)<- gsub(".h5ad", "", colnames(metrics))

# simu <- c("Mono-kinetic","Transcriptional_boost","Multi-forward_branching","Multi-backward_branching")
# simu <- paste0(simu,"_",rep(1:10,each=4))
simu <-  c(paste0("Mono-kinetic_",1:10),
           paste0("Transcriptional_boost_",1:10),
           paste0("Multi-forward_branching_",1:10),
           paste0("Multi-backward_branching_",1:10))

metrics <- metrics[,c('Method',simu)]
fwrite(metrics,'/media/liyaru/LYR/Benchmark/simu_celldancer/celldancer_cosine_sim_all_reorder.csv')


# add median
metrics$Median <- apply(metrics[,2:41], 1, median, na.rm = TRUE)
metrics$Mean <- apply(metrics[,2:41], 1, mean, na.rm = TRUE)

metrics <- as.data.frame(metrics)

# normalize by group
x_min <- min(metrics[,2:41], na.rm = TRUE)
x_max <- max(metrics[,2:41], na.rm = TRUE)
metrics[,2:41] <- (metrics[,2:41] - x_min) / (x_max - x_min)
# 
# # normalize mean and median
# x_min <- min(metrics[,42:43], na.rm = TRUE)
# x_max <- max(metrics[,42:43], na.rm = TRUE)
# metrics[,42:43] <- (metrics[,42:43] - x_min) / (x_max - x_min)
...
ggsave("/media/liyaru/LYR/Benchmark/Fig/5.simu_cd_cosine.pdf",
       g, 
